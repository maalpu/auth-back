...
export const updateProduct = async (req, res) => {
  try {
    const {id} = req.params
    const {barcode, prod_name, measures, prod_type, id_categ, stock, stock_min, stock_ideal, detail, url_img1, url_img2, url_img3, url_img4, url_img5, price, destacade, active} = req.body
    const [result] = await pool.query(`UPDATE products SET barcode = IFNULL(?, barcode), prod_name = ISNULL(?, prod_name), measures = ISNULL(?, measures), prod_type = ISNULL(?, prod_type), id_categ = ISNULL(?, id_categ), stock = ISNULL(?, stock), stock_min = ISNULL(?, stock_min), stock_ideal = ISNULL(?, stock_ideal), detail = ISNULL(?, detail), url_img1 = ISNULL(?, url_img1), url_img2 = ISNULL(?, url_img2), url_img3 = ISNULL(?, url_img3), url_img4 = ISNULL(?, url_img4), url_img5 = ISNULL(?, url_img5), price = ISNULL(?, price), destacade = ISNULL(?, destacade), active = ISNULL(?, active) where id_prod `, [barcode, prod_name, measures, prod_type, id_categ, stock, stock_min, stock_ideal, detail, url_img1, url_img2, url_img3, url_img4, url_img5, price, destacade, active, id])
